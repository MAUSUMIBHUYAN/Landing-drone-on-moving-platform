...
class FollowDescentDroneController(Node):
    def __init__(self):
        ...
        self.cart_front_offset = 0.35; self.prediction_latency = 0.15
    ...
    def control_loop(self):
        ...
        if self.state not in ["LANDED", "TAKEOFF"]:
            ...
            self.plot_x.append(self.drone_position[0])
            self.plot_y.append(self.drone_position[1])
            self.plot_z.append(self.drone_position[2])
    ...
    def handle_landing(self, current_time):
        ...
        if self.drone_position[2] <= self.robot_height + self.land_altitude_threshold:
            self.land_pub.publish(Empty()); self.state = "LANDED"; self.get_logger().info("State: LANDED - Final land command sent.")
            self.generate_plots()
            if len(self.plot_time) > 2:
                self.generate_time_animation()   # animation
            else:
                self.get_logger().warn("Not enough data to generate animation.")  # ⬅️ animate x(t), y(t), z(t)
    ...
    def shutdown_hook(self):
        ...
        if self.takeoff_timer and not self.takeoff_timer.is_canceled():
    ...
    def generate_plots(self):
        ...
        # X vs Time
        plt.figure()
        plt.plot(self.plot_time, self.plot_x, label='x(t)')
        plt.scatter(self.plot_time[0], self.plot_x[0], marker='o', label='Start')
        plt.scatter(self.plot_time[-1], self.plot_x[-1], marker='x', label='End')
        plt.xlabel('Time (s)')
        ...
        # Y vs Time
        plt.figure()
        plt.plot(self.plot_time, self.plot_y, label='y(t)')
        plt.scatter(self.plot_time[0], self.plot_y[0], marker='o', label='Start')
        plt.scatter(self.plot_time[-1], self.plot_y[-1], marker='x', label='End')
        plt.xlabel('Time (s)')
        ...
        # Z vs Time
        plt.figure()
        plt.plot(self.plot_time, self.plot_z, label='z(t)')
        plt.scatter(self.plot_time[0], self.plot_z[0], marker='o', label='Start')
        plt.scatter(self.plot_time[-1], self.plot_z[-1], marker='x', label='End')
        plt.xlabel('Time (s)')
        ...
        plt.grid(True)
        plt.show()
    def generate_time_animation(self):

        if len(self.plot_time) < 2:
            print("⚠️ Not enough data collected for animation. Skipping GIF generation.")
            return
        ...
        line_x, = ax[0].plot([], [], lw=2)
        line_y, = ax[1].plot([], [], lw=2)
        line_z, = ax[2].plot([], [], lw=2)

        start_x = ax[0].plot([], [], 'go', markersize=6)[0]
        end_x = ax[0].plot([], [], 'rx', markersize=8)[0]
        start_y = ax[1].plot([], [], 'go', markersize=6)[0]
        end_y = ax[1].plot([], [], 'rx', markersize=8)[0]
        start_z = ax[2].plot([], [], 'go', markersize=6)[0]
        end_z = ax[2].plot([], [], 'rx', markersize=8)[0]
        ...
        def update(frame):
            if frame == 0 or len(self.plot_time) < 2:
                return line_x, line_y, line_z  # Skip updating markers on first frame

            t_vals = self.plot_time[:frame]
            x_vals = self.plot_x[:frame]
            y_vals = self.plot_y[:frame]
            z_vals = self.plot_z[:frame]
            ...
            # Axis limits
            for axis in ax:
                axis.set_xlim(0, max(self.plot_time))
            ax[0].set_ylim(min(self.plot_x) - 0.2, max(self.plot_x) + 0.2)
            ax[1].set_ylim(min(self.plot_y) - 0.2, max(self.plot_y) + 0.2)
            ax[2].set_ylim(min(self.plot_z) - 0.2, max(self.plot_z) + 0.2)

            # Only update markers if valid
            start_x.set_data([self.plot_time[0]], [self.plot_x[0]])
            end_x.set_data([t_vals[-1]], [x_vals[-1]])

            start_y.set_data([self.plot_time[0]], [self.plot_y[0]])
            end_y.set_data([t_vals[-1]], [y_vals[-1]])

            start_z.set_data([self.plot_time[0]], [self.plot_z[0]])
            end_z.set_data([t_vals[-1]], [z_vals[-1]])

            return line_x, line_y, line_z


        ani = animation.FuncAnimation(
            fig, update, frames=len(self.plot_time), init_func=init, blit=True, interval=100
        )

        ani.save('drone_time_series.gif', writer='pillow')
        plt.close(fig)
        print("🎉 Time series animation saved as drone_time_series.gif")
