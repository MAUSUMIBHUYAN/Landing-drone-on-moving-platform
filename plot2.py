...
class PlanarVehicleKF(KalmanFilter):
    ...
    def __init__(self, dt, q_accel=1.5, r_meas=5.0):
...
class FollowDescentDroneController(Node):
    def __init__(self):
        ...
        # --- MODIFICATION 1: Low Noise KF Parameters ---
        # Setting Q and R to very small values tells the filter to treat the prediction (Q) 
        # as perfect and the measurement (R) as highly reliable.
        LOW_Q_ACCEL = 0.001  # Reduced from 1.5
        LOW_R_MEAS = 0.01    # Reduced from 5.0

        self.kf_xy = PlanarVehicleKF(self.dt, q_accel=LOW_Q_ACCEL, r_meas=LOW_R_MEAS)
        ...
        self.log_data = []
        self.log_filename = 'robot_trajectory_low_noise_log.csv' # Changed filename for clarity
        # -------------------------------
    ...
    def robot_pose_callback(self, msg: Odometry):
        ...
        # --- MODIFICATION 2: World Frame Velocity Calculation for Logging (If using the velocity plotting script) ---
        ...
        # --- Log the data point (includes world-frame velocity if needed for plotting) ---
        self.log_data.append({
            'time': current_time - self.start_time,
            'actual_x': new_pos[0],
            'actual_y': new_pos[1],
            'estimated_x': self.robot_position[0],
            'estimated_y': self.robot_position[1],
            'drone_x': self.drone_position[0],
            'drone_y': self.drone_position[1],
            'actual_vx': vx_true_world,
            'actual_vy': vy_true_world,
            'estimated_vx': self.robot_velocity[0],
            'estimated_vy': self.robot_velocity[1],
        })
        # ------------------------------------------------------------------------------
    ...
    def shutdown_hook(self):
        ...
        if self.log_data:
            ...
            self.get_logger().info(f"Trajectory data saved to {self.log_filename}")
        # -------------------------
        
        if self.takeoff_timer and not self.takeoff_timer.cancelled(): self.takeoff_timer.cancel()
        if self.control_timer and not self.control_timer.cancelled(): self.control_timer.cancel()
